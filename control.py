...
def get_signed_angle_diff(vector1, vector2):#计算辆车朝向角之差，返回单位为度，范围为-180- +180
...
def get_steer_angle_range(actor):#获得 前轮转角 = 左前轮最大转向角和右前轮最大专项较的平均值
    actor_physics_control = actor.get_physics_control()#得到一个存储控制车辆参数的对象carla.VehiclePhysicsControl
    return (actor_physics_control.wheels[0].max_steer_angle + actor_physics_control.wheels[1].max_steer_angle) / 2
            #VehiclePhysicsControl.wheels: (list(carla.WheelPhysicsControl))[FL, FR, BL, BR]
...
def get_acceleration(actor):
    ...
    return carla.Vector2D(acc.x, acc.y)
...
def do_speed_statistics(c, car_agents, bike_agents, pedestrian_agents, statistics):
    ...
    for agent in car_agents:
        ...
        avg_acc_cars+=get_acceleration(agent.actor).length()
    ...
    statistics.avg_acc_cars=avg_acc_cars
    statistics.avg_speed_bikes = avg_speed_bikes
    statistics.avg_speed_pedestrians = avg_speed_pedestrians
    for agent in car_agents:
        pos3d=[get_position_3d(agent.actor).x,get_position_3d(agent.actor).y]
        if agent.actor.id in statistics.pos_car.keys():
            statistics.pos_car[agent.actor.id]=pos3d
        else:
            statistics.pos_car[agent.actor.id]=[]
            statistics.pos_car[agent.actor.id]=pos3d

    for agent in bike_agents:
        pos3d=[get_position_3d(agent.actor).x,get_position_3d(agent.actor).y]
        if agent.actor.id in statistics.pos_bike.keys():
            statistics.pos_bike[agent.actor.id]=pos3d
        else:
            statistics.pos_bike[agent.actor.id]=[]
            statistics.pos_bike[agent.actor.id]=pos3d
    return statistics
